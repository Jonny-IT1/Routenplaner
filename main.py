...
class RouteFinderApp:
    ...
    def create_main_layout(self):
        ...
        tk.OptionMenu(sidebar, self.start_var, *CITIES.keys(), command=self.on_dropdown_change).pack(fill="x", pady=2)
        tk.Label(sidebar, text="Ziel:", bg=self.bg_sidebar, fg=self.fg_white).pack(anchor="w")
        tk.OptionMenu(sidebar, self.dest_var, *CITIES.keys(), command=self.on_dropdown_change).pack(fill="x", pady=2)
